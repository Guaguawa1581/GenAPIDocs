...
import { parseResponseJson, flattenFields, unflattenFields } from '../../utils/parseJson';
...
const handleResponseChange = (resp: ApiResponse, isManual = false) => {
  if (resp.rawJson && (resp.fields.length === 0 || isManual)) {
    try {
      ...
      const existingMap = new Map(currentFlat.map(f => [f.name, f.description]));
      ...
      resp.fields = newTree;
      if (isManual) {
          toast.add({ severity: 'info', summary: '解析完成', detail: '已重新根據 JSON 刷新欄位', life: 2000 });
      }
    } catch (e) {
      toast.add({
        severity: 'error',
...
const deleteField = (resp: ApiResponse, id: string) => {
    ...
    let count = 1;
    while (index + count < allFlat.length && allFlat[index+count].level! > target.level!) {
        count++;
    }
...
const onRowReorder = (event: DataTableRowReorderEvent, resp: ApiResponse) => {
    ...
    if (!sourceRow.isCustom) {
        if (targetRow.level !== sourceRow.level) {
            toast.add({ severity: 'warn', summary: '移動受限', detail: '非自訂欄位僅能在同階層移動', life: 2000 });
            return;
        }
        ...
        for (let k = startIndex + 1; k < endIndex; k++) {
            if (allFlat[k].level! < sourceRow.level!) {
                toast.add({ severity: 'warn', summary: '移動受限', detail: '不可跨越父項目', life: 2000 });
                return;
            }
        }
    }
...
<template>
  <div class="step-container mw-100">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <Button label="新增 Response 狀態" icon="pi pi-plus" size="small" @click="addResponse" />
    </div>

    <Tabs v-model:value="activeTabIndex">
      <TabList>
        <Tab v-for="(resp, index) in draftStore.config.responses" :key="resp.id" :value="index">
      ...
      <TabPanels>
        <TabPanel v-for="(resp, index) in draftStore.config.responses" :key="resp.id" :value="index">
          <div class="response-editor-grid mt-3">
            <div class="resp-header mb-4 p-3 bg-light rounded border shadow-sm">
              <div class="row g-3">
                <div class="col-md-2">
                  <label class="form-label fw-bold small text-secondary">HTTP Code</label>
                  <InputText v-model="resp.statusCode" type="number" class="w-100 p-2" />
                </div>
                <div class="col-md-4">
                  <label class="form-label fw-bold small text-secondary">Status Text</label>
                  <InputText v-model="resp.statusText" class="w-100 p-2" />
                </div>
                <div class="col-md-6">
                  <label class="form-label fw-bold small text-secondary">說明 / 備註 (Excel 標記用)</label>
                  <InputText v-model="resp.note" class="w-100 p-2" placeholder="例如：發生查無資料時" />
                </div>
              </div>
            </div>
            ...
            <div class="split-layout">
              <div class="json-side">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <Button label="重新解析欄位" icon="pi pi-refresh" size="small" variant="text" @click="handleResponseChange(resp, true)" />
                </div>
                <Textarea 
              ...
              <div class="fields-side">
                <div class="d-flex justify-content-between align-items-center mb-2">
                  ...
                  <Button label="新增自訂欄位" icon="pi pi-plus-circle" size="small" text @click="addCustomField(resp)" />
                </div>
                ...
                >
                  ...
                  <Column header="欄位名稱 (Key)" style="min-width: 15rem">
                    <template #body="{ data }">
                      <div :style="{ paddingLeft: (data.level * 1.5) + 'rem' }" class="d-flex align-items-center gap-1">
                        <Button 
                            v-if="isComplex(data.type)" 
                            :icon="data.expanded ? 'pi pi-chevron-down' : 'pi pi-chevron-right'" 
                            variant="text" 
                            class="p-0" 
                            style="width: 1.25rem; height: 1.25rem; flex-shrink: 0;" 
                            @click="toggleExpand(data)" 
                        />
                        <span v-else-if="data.level > 0" class="me-1 text-secondary opacity-50">└</span>
                        
                        <div class="position-relative w-100">
                            <InputText v-model="data.name" class="p-1 font-monospace w-100 border-0 bg-transparent edit-focus" />
                            <div class="bottom-line"></div>
                        </div>
                        <Tag v-if="isComplex(data.type)" severity="info" size="small" value="Obj" />
                        <Tag v-if="data.isCustom" severity="warn" size="small" value="User" />
                      </div>
                    </template>
                  </Column>
                  ...
                  <Column header="意思 (Description)" style="width: 15rem">
                    <template #body="{ data }">
                      <InputText v-model="data.description" class="w-100 p-1 border-0 border-bottom rounded-0" placeholder="說明文字" />
                    </template>
                  </Column>
                  <Column header="操作" style="width: 4rem">
                    <template #body="{ data }">
                        <Button icon="pi pi-trash" variant="text" severity="danger" size="small" @click="deleteField(resp, data.id)" />
                    </template>
                  </Column>
                </DataTable>
              </div>
