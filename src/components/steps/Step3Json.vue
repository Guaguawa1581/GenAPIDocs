...
import { parseRequestJson, flattenFields, unflattenFields } from '../../utils/parseJson';
...
const displayedFields = computed(() => getVisibleFields(flattenFields(draftStore.config.requestFields)));
    ...
    <div class="split-layout mt-3">
      <div class="json-side">
        <div class="d-flex justify-content-between align-items-center mb-2">
            <label class="fw-bold text-primary">Request JSON 範例</label>
            <Button label="重新解析欄位" icon="pi pi-refresh" size="small" variant="text" @click="handleRequestChange(true)" />
        </div>
        <Textarea 
      ...
      <div class="fields-side">
        <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <Button label="新增自訂欄位" icon="pi pi-plus-circle" size="small" text @click="addCustomField" />
        </div>
        ...
        >
          ...
          <Column header="欄位名稱 (Key)" style="min-width: 14rem">
            <template #body="{ data }">
              <div :style="{ paddingLeft: (data.level * 0.5) + 'rem' }" class="d-flex align-items-center gap-1">
                <Button 
          ...
          <Column header="操作" style="width: 4rem">
            <template #body="{ data }">
              <Button icon="pi pi-trash" variant="text" severity="danger" size="small" @click="deleteField(data.id)" />
            </template>
          </Column>
        </DataTable>
        
        <div class="mt-3 p-2 bg-light border rounded small text-secondary">
          💡 自訂欄位移至物件下方會自動併入該物件。目前儲存結構：TreeNode { children: [] }。
        </div>
      </div>
    </div>
  </div>
